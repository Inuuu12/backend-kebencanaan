...
export default function PublicLayout({ children, mapMode = false }) {
    const { user } = useAuth();
    ...
    return (
        <div className={`min-h-screen ${mapMode ? 'h-screen overflow-hidden' : ''} bg-[#FDFDFC] dark:bg-[#0a0a0a] text-[#1b1b18] dark:text-[#EDEDEC] font-sans transition-colors duration-300 relative`}>
            {/* Header */}
                ...
                <div className="container mx-auto px-6 h-16 flex items-center justify-between gap-4">
                    ...
                    <nav className="flex items-center gap-3 sm:gap-6 text-sm font-medium shrink-0">
                        ...
                        >
                            ...
                            <span className={isWeatherOpen ? 'text-blue-500 font-bold' : ''}>Cuaca</span>
                            <ChevronDown size={14} className={`transition-transform duration-300 ${isWeatherOpen ? 'rotate-180 text-blue-500' : ''}`} />
                        </button>
                        {user ? (
                            <Link to={(user.role === 'superadmin') ? '/dashboard/kabupaten' : user.role === 'admin_kecamatan' ? '/dashboard/kecamatan' : '/dashboard/kelurahan'} className="btn-primary rounded-full px-5 py-2">
                                Masuk Dashboard
                            </Link>
                        ) : (
                            <Link to="/login" className="relative group p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-600 dark:text-slate-300 flex items-center justify-center">
                                <User size={22} />
                                {/* Tooltip */}
            ...
            {/* Main Content */}
            <main className={mapMode ? 'h-full pt-16 relative' : ''}>
