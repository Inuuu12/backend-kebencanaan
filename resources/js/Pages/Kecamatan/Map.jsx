...
export default function Map({ district, villages, complaints }) {
    ...
    const filteredComplaints = complaints.filter(c => {
        ...
        return matchesVillage && matchesType && matchesStatus && matchesPriority;
    });

    useEffect(() => {
        if (typeof window === 'undefined') return;

        // Centroid of Babakan Madang
        const map = L.map('kecamatan-map-full', {
            zoomControl: false
        ...
        filteredComplaints.forEach((c) => {
            ...
            marker.bindPopup(`
                <div style="color: #0f172a; padding: 0.25rem; min-width: 150px;">
                    ...
                    <a href="/dashboard/kecamatan/aduan" style="display: block; font-size: 0.75rem; color: #3b82f6; font-weight: 600; margin-top: 0.5rem; text-decoration: none;">Kelola Laporan &rarr;</a>
                </div>
            `);
    ...
    return (
        <Layout activePage="map" title={`Peta Geografis Kecamatan ${district.name}`}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '3fr 1fr', gap: '1.5rem', height: 'calc(100vh - 160px)' }}>
                ...
                <div className="panel-card" style={{ padding: '0', position: 'relative', overflow: 'hidden' }}>
                    <div id="kecamatan-map-full" style={{ width: '100%', height: '100%' }}></div>
                ...
                <div className="panel-card" style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                    ...
                    <div className="form-group">
                        <label htmlFor="map-v"><Filter size={10} style={{ marginRight: '4px' }} /> Wilayah Desa</label>
                        <select 
                            id="map-v" 
                    ...
                    <div style={{ borderTop: '1px solid var(--border-color)', paddingTop: '1.25rem', flexGrow: 1 }}>
                        <h4 style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: '0.75rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Warna Wilayah (Status Desa)</h4>
