...
export default function Complaints({ districts, villages, complaints }) {
    ...
    // Dynamically filter villages based on selected district
    const filteredVillagesDropdown = useMemo(() => {
        if (!districtFilter) return villages;
        return villages.filter(v => v.district_id === parseInt(districtFilter));
    }, [districtFilter, villages]);

    // Handle district filter change
    const handleDistrictFilterChange = (e) => {
        setDistrictFilter(e.target.value);
        setVillageFilter(''); // reset village filter
    };
    ...
    const filteredComplaints = useMemo(() => {
        return complaints.filter(c => {
            ...
            const matchesDistrict = districtFilter ? (c.village?.district_id === parseInt(districtFilter)) : true;
            const matchesVillage = villageFilter ? (c.village_id === parseInt(villageFilter)) : true;
            ...
            return matchesSearch && matchesDistrict && matchesVillage && matchesStatus && matchesPriority && matchesType;
        });
    }, [searchTerm, districtFilter, villageFilter, statusFilter, priorityFilter, typeFilter, complaints]);
    ...
    return (
        <Layout activePage="complaints" title="Pusat Monitoring Laporan Bencana Se-Kabupaten">
                            ...
                            {filteredComplaints.length === 0 ? (
                                ...
                                filteredComplaints.map((c) => (
                                    <tr key={c.id}>
                                        ...
                                        <td>
                                            <div style={{ fontWeight: 600 }}>Kel. {c.village?.name}</div>
                                            <div style={{ fontSize: '0.7rem', color: 'var(--text-secondary)' }}>Kec. {c.village?.district?.name}</div>
                                        </td>
            ...
            {/* Modal Detail & Management (Read Only) */}
            {selectedComplaint && (
                <div className="modal-overlay">
                    <div className="modal-content animate-scale-up">
                        <div className="modal-header">
                            <h3>Aduan #{selectedComplaint.id}: <span style={{ textTransform: 'capitalize', color: 'var(--color-primary)' }}>{selectedComplaint.disaster_type}</span> (Kel. {selectedComplaint.village?.name}, Kec. {selectedComplaint.village?.district?.name})</h3>
                        ...
                        <div className="modal-body">
                            ...
                            {activeTab === 'details' && (
                                <div>
                                    ...
                                    <div className="form-group" style={{ marginBottom: '1.5rem', marginTop: '1.5rem' }}>
                                        ...
                                        <p style={{ background: 'rgba(255,255,255,0.03)', padding: '1rem', borderRadius: '8px', fontSize: '0.9rem', lineHeight: '1.5', border: '1px solid var(--border-color)', margin: 0 }}>
                            ...
                            {activeTab === 'handlings' && (
                                <div>
                                    <h4 style={{ fontSize: '1rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <Activity size={16} className="color-primary" /> Log Progres Lapangan
                                    </h4>

                                    <div>
                                        {!selectedComplaint.handlings || selectedComplaint.handlings.length === 0 ? (
                                            <div style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem 0' }}>Belum ada log penanganan lapangan.</div>
                                        ) : (
                            ...
                            {activeTab === 'damages' && (
                                <div>
                                    <h4 style={{ fontSize: '1rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <Hammer size={16} className="color-primary" /> Infrastruktur Terdampak
                                    </h4>
                            ...
                            {activeTab === 'logistics' && (
                                <div>
                                    <h4 style={{ fontSize: '1rem', marginBottom: '1rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                        <PackageOpen size={16} className="color-primary" /> Bantuan Logistik
                                    </h4>
                                    ...
                                    <table className="custom-table">
                                        ...
                                        <tbody>
                                            {!selectedComplaint.logistics || selectedComplaint.logistics.length === 0 ? (
                                                <tr>
                                                    <td colSpan="3" style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '2rem 0' }}>Tidak ada pengajuan logistik untuk aduan ini.</td>
                                                </tr>
                                            ) : (
            ...
            {familyModalResident && (() => {
                const members = familyModalResident.family_members || [];
